...
export default function Music({ forceMusicPage = false, providerClient = null, playbackQuality = '128k', storageNamespace = '', favoriteNamespace = '', favoriteBackup = [], favoriteBackupUpdatedAt = null, favoriteSyncEnabled = false, onRequireAccess, pageTitle = forceMusicPage ? 'LURI MUSIC' : TEXT.title, headerNotice = null, legalLinks = null }) {
  ...
  const lyricsRef = useRef(null); const musicListRef = useRef(null); const currentRowRef = useRef(null); const searchFormRef = useRef(null); const searchSuggestionsRef = useRef(null); const locatedTrackRef = useRef('');
  const progressInputRef = useRef(null); const progressFrameRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const hideSearchHistory = (event) => {
      if (searchFormRef.current?.contains(event.target)) return;
      setSearchFocused(false);
      setActiveSingerSuggestion(-1);
    };
    document.addEventListener('pointerdown', hideSearchHistory);
    return () => document.removeEventListener('pointerdown', hideSearchHistory);
  }, [searchFocused]);
  ...
  useEffect(() => {
    ...
    const renderProgress = () => {
      const player = audio.current; const input = progressInputRef.current;
      if (player && input) {
        const mediaDuration = Number.isFinite(player.duration) ? player.duration : duration;
        const mediaProgress = Math.min(Math.max(player.currentTime || 0, 0), mediaDuration || 0);
        input.value = String(mediaProgress);
        document.documentElement.style.setProperty('--music-progress', `${mediaDuration ? mediaProgress / mediaDuration * 100 : 0}%`);
      }
      progressFrameRef.current = window.requestAnimationFrame(renderProgress);
    };
    progressFrameRef.current = window.requestAnimationFrame(renderProgress);
    return () => {
  ...
  <section className="music-shell">
    ...
    <footer className="music-player">
      <div className="music-player__progress"><input ref={progressInputRef} type="range" min="0" max={duration || 0} step="0.01" value={Math.min(progress, duration || 0)} aria-label="播放进度" onChange={(event) => { const value = Number(event.target.value); if (audio.current) audio.current.currentTime = value; setProgress(value); }} /></div>
