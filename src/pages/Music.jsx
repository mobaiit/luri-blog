...
function DownloadButton({ track }) {
  const href = track?.downloadUrl || track?.url;
  ...
  return <a className="music-download-button" href={href} download={filename} target="_blank" rel="noreferrer" aria-label={`下载 ${track.title}`} title="下载音频；若浏览器不支持跨域下载，将打开源文件"><MusicIcon name="download" /></a>;
}
...
export default function Music({ forceMusicPage = false, providerClient = null, playbackQuality = 'auto', storageNamespace = '', onRequireAccess, pageTitle = forceMusicPage ? 'LURI MUSIC' : TEXT.title, headerNotice = null, legalLinks = null }) {
  ...
  useEffect(() => {
    ...
    musicRequest('resolve', { id: sourceTrackId(current), source: current.source || '', title: current.title || '', artist: current.artist || '', quality: playbackQuality, meta: current.meta || undefined, fallbackOnly: forceLocalFallbackFor === current.id ? '1' : '' }, controller.signal).then((response) => response.ok ? response.json() : {}).then((payload) => {
      ...
      if (!controller.signal.aborted) {
        const update = (track) => track.id === current.id ? { ...track, url: payload.url, downloadUrl: payload.downloadUrl || '', art: payload.art || track.art, requestedQuality: payload.requestedQuality || playbackQuality, quality: payload.quality || '', bitrate: payload.bitrate || null, degraded: Boolean(payload.degraded), qualityVerified: Boolean(payload.qualityVerified) } : track;
  ...
  const playResult = async (result) => {
    ...
    try {
      ...
      const track = { ...result, sourceId: rawId, id: trackKey({ ...result, id: rawId }, storageNamespace), url: payload.url, downloadUrl: payload.downloadUrl || '', art: payload.art || result.art || '', requestedQuality: payload.requestedQuality || playbackQuality, quality: payload.quality || '', bitrate: payload.bitrate || null, degraded: Boolean(payload.degraded), qualityVerified: Boolean(payload.qualityVerified) };
