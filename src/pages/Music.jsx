...
export default function Music() {
  ...
  const [tracks, setTracks] = useState(() => readStore(STORE_QUEUE)); const [likedTracks, setLikedTracks] = useState(() => readStore(STORE_LIKES)); const [currentId, setCurrentId] = useState(null); const [playing, setPlaying] = useState(false); const [titleOverflows, setTitleOverflows] = useState(false);
  ...
  return <main className={`music-page${playing ? ' is-playing' : ''}${titleOverflows ? ' has-overflowing-title' : ''}`}><audio ref={audio} onPlay={() => { setPlaying(true); setMobileView('now'); }} onPause={() => setPlaying(false)} onLoadStart={() => setSearchState('正在加载音频…')} onWaiting={() => setSearchState('正在加载音频…')} onPlaying={() => setSearchState('')} onError={() => setSearchState('音频加载失败，请尝试其他歌曲')} onTimeUpdate={(event) => setProgress(event.currentTarget.currentTime)} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onEnded={next} /><nav className="music-mobile-tabs" aria-label="移动端音乐视图"><button className={mobileView === 'playlist' ? 'active' : ''} onClick={() => setMobileView('playlist')}>播放列表</button><button className={mobileView === 'now' ? 'active' : ''} onClick={() => setMobileView('now')}>正在播放</button></nav>
