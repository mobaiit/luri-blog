...
const STORE_QUEUE = 'luri.music.queue.v1'; const STORE_LIKES = 'luri.music.likes.v1';
...
export default function Music() {
  ...
  const [query, setQuery] = useState(''); const [results, setResults] = useState([]); const [cachedResults, setCachedResults] = useState([]); const [searchState, setSearchState] = useState('');
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ title: current.title, artist: current.artist || '', album: current.album || '' });
  ...
  <section className="music-shell"><aside className="music-sidebar"><p className="music-brand">LURI / MUSIC</p><button className={`music-nav${listView === 'search' ? ' active' : ''}`} onClick={() => { setListView('search'); setResults(cachedResults); }}>播放列表<span>{cachedResults.length || tracks.length}</span></button><button className={`music-nav${listView === 'likes' ? ' active' : ''}`} onClick={() => { setListView('likes'); setResults(likedTracks); }}>我喜欢<span>{liked.size}</span></button><div className="music-divider" /></aside>
