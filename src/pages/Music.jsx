...
export default function Music() {
  ...
  const playlistScrollTop = useRef(0);
  ...
  const switchMobileView = (view) => {
    const playlist = document.querySelector('.music-page .music-list');
    if (mobileView === 'playlist' && playlist) playlistScrollTop.current = playlist.scrollTop;
    setMobileView(view);
    if (view === 'playlist') requestAnimationFrame(() => { const list = document.querySelector('.music-page .music-list'); if (list) list.scrollTop = playlistScrollTop.current; });
  };

  return <main className={`music-page mobile-view-${mobileView}${playing ? ' is-playing' : ''}${titleOverflows ? ' has-overflowing-title' : ''}`}><audio ref={audio} onPlay={() => { setPlaying(true); setPlaybackTrackId(currentId); setPlaybackState('playing'); switchMobileView('now'); }} onPause={() => { setPlaying(false); setPlaybackState((state) => state === 'loading' || state === 'resolving' ? state : 'paused'); }} onLoadStart={() => setPlaybackState('loading')} onWaiting={() => setPlaybackState('loading')} onPlaying={() => setPlaybackState('playing')} onError={() => { setPlaying(false); setPlaybackState('error'); }} onTimeUpdate={(event) => setProgress(event.currentTarget.currentTime)} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onEnded={next} /><nav className="music-mobile-tabs" aria-label="移动端音乐视图"><button className={mobileView === 'playlist' ? 'active' : ''} onClick={() => switchMobileView('playlist')}>播放列表</button><button className={mobileView === 'now' ? 'active' : ''} onClick={() => switchMobileView('now')}>正在播放</button></nav>
